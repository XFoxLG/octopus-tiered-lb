...
- 📊 **Analytics 与 Evaluation** - 提供缓存概览、供应商 / 模型 / API Key 利用率、路由健康、延迟分布、供应商 Prompt Cache 分析，以及分组测试 / AI 路由入口
- 🛠️ **Ops 与审计** - 提供遥测、配额、健康、系统、审计面板，以及管理面写操作审计链路
- **Redis / Valkey 配置** - 粘贴连接串即可识别、测试；Render 按服务加密存库，重启生效，保留环境变量优先级
...
大多数运行时调优项不存放在 `config.json` 中。重试策略、熔断阈值、Auto 策略调优、日志保留周期、对外 API 基础地址、AI 路由服务配置、WebDAV 备份、代理池、模型映射规则等，都通过设置页 / 管理 API 动态写入数据库。

本分支已退役 Semantic Cache（语义答案复用）的界面和执行链；普通 embeddings、rerank、AI 分组和上游 Prompt Cache 保留。不会删除共享数据库里的旧语义设置或历史日志，旧开关也不会重新启用实验版的语义缓存。

Redis 可在设置页粘贴 `redis://` 或 `rediss://` 连接串，后者自动开启证书校验的 TLS。Render 上使用后端的稳定 `RENDER_SERVICE_ID` 加密保存整份配置，重启对应服务后生效，不在保存时切断现有连接。显式 `OCTOPUS_CACHE_*` 环境变量优先；`OCTOPUS_CACHE_TYPE=` 表示使用内存。要改用网页存库，应先移除该服务的这些覆盖变量并重新部署。没有有效 Render 服务 ID 时，普通文件部署仍沿用本地配置方式。

网页不回传已存密码；留空新连接输入框表示保留，填新链接表示完整替换，不继承旧密码。Aiven 的 `CLICK_TO:REVEAL_PASSWORD` 是占位内容，需私下复制真实连接串并确认服务已启动。Render 内网链接要求同一 workspace、同一区域，两个账号不能默认互通。

配置只在启动和显式保存时处理，不新增每条聊天请求的查库、连接池或轮询任务。普通 JSON/WebDAV 备份不携带服务专属 Redis 凭据；全库恢复仍需保留原加密密钥。不同域名共用 SQL 不等于环境隔离，同一个 Redis 地址/DB 仍共享运行状态键。现有迁移 059 会删除旧站点表，上线前必须在副本核对混用版本的兼容性，不能把本次新增配置表等同于整个启动过程无损。详见[部署、缓存与聊天兼容说明](docs/deployment-cache-and-chat.md)。
...
| Analytics | 渠道×模型（默认）、用量拆分、路由健康、延迟分布、评估中心、供应商 Prompt Cache 和分享快照 |
...
| Setting | 版本更新信息、外观与导航偏好（排序 + 可见性）、运行时调优、AI 路由服务池、WebAuthn/Passkey、Redis 连接配置、WebDAV 备份、备份恢复和模型名归一化规则 |
| User | 管理员用户和角色管理 |
