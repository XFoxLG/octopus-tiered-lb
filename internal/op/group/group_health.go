...
// GetLatestSnapshotByGroupID 取分组最新快照（含拨测明细）。
...
// staleRunningSnapshotAge 判定 running 快照为崩溃遗留的阈值。
const staleRunningSnapshotAge = 10 * time.Minute

// GetRunningSnapshotByGroupID 取分组未超阈值的 running 快照（防并发重复检查）。
...
// ListGroupHealthViews 返回全部分组与其最新快照的聚合视图。
...
// GetGroupHealthViewByID 取单分组聚合视图（无快照时 Latest 为空）。
