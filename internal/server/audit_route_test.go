...
var exemptFromAudit = map[string]string{
	...
	"POST /api/v1/group/health/run/:id":         "group health probe — writes health snapshots only",
	"POST /api/v1/group/health/run-all":         "group health probe — writes health snapshots only",
